...
export function PhotoCapture({ disabled, onCapture }: { disabled: boolean; onCapture: (file: File) => void }) {
  ...
  function close() { stream.current?.getTracks().forEach(track => track.stop()); stream.current = null; if (video.current) video.current.srcObject = null; setOpen(false); }
  async function start() {
    ...
    try {
      ...
      stream.current = input; setOpen(true);
  ...
  return <div className="photo-capture">
    {!open && <button type="button" className="button button-outline" disabled={disabled || pending} onClick={() => void start()}>{pending ? "Ouverture de la caméra…" : "Ouvrir la caméra"}</button>}
    <video ref={video} hidden={!open} muted playsInline aria-label="Aperçu de votre caméra" />
    {open && <div className="media-actions"><button type="button" className="button button-sun" onClick={snap} disabled={disabled || pending}>Prendre cette photo</button><button type="button" className="button button-outline" onClick={close}>Fermer la caméra</button></div>}
